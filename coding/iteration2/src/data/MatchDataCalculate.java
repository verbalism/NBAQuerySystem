...
public class MatchDataCalculate {
	...
	public int BasicOneMatchOriginal(MatchPO mp) throws Exception{  
	 	//在数据库中读入比赛基本信息
		int result=0;
		String teamName1=mp.getTeam1().getTeamName();
		String date=mp.getMatchTime();
	 	File f=new File("");
	 	String s=f.getCanonicalPath();
	 	Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");  
        String dbur1 = "jdbc:odbc:driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ="+s+"//NBAIteration2";  
        ...
        Connection conn = DriverManager.getConnection(dbur1, "username", "password"); 
        Statement stmt=conn.createStatement();
        ResultSet rs=stmt.executeQuery("select * from generalMatchInfo where teamName1='"+teamName1+"' and matchTime='"+date+"'");
        while(rs.next()){
        	result=-1;
        }
        if(result!=-1){
