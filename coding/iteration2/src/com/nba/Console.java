	...
	public  void execute(PrintStream out,String[] args) {
		...
		for(int i=0;i<args.length;i++){
			...
			while(j<cmds.length){
				...
				if(cmds[j].equals("-n")){
					cp.parse(new String[]{cmds[j],cmds[j+1]});
					j++;
				}
				else if(cmds[j].equals("-fliter")||cmds[j].equals("-sort")||cmds[j].equals("-king")||cmds[j].equals("-hots")){
					cp.parse(new String[]{cmds[j],cmds[j+1]});
					j++;
				}
				else{
			...
			if(cmds[0].equals("-player")){
				...
				/**筛选全部球员*/
					...
					if(player.getSortField){
						...
						field = new Object[][]{{"trueShootingPercentage",SortType.Descending}};
					else
						field = new Object[][]{{"points",SortType.Descending}};
					
					//players = abl.getTopFiftyPlayer(player.n, player.position, player.league, player.age, field);
					...
					if(player.highData){
						for(int k=0;k<players.size();k++){
							...
							//highPlayer.setLeague(players.get(k).);
						...
						if(!player.avgValue){
							for(int k=0;k<players.size();k++){
								...
								//normal.setMinute(players.get(k).getMinutes());
							...
							for(int k=0;k<players.size();k++){
								...
								//normal.setMinute(players.get(k).getMinutes()/players.get(k).getGamesPlayed());
			...
			if(cmds[0].equals("-team")){
				...
				if(team.hotTeams){
					...
					for(int k=0;k<teams.size();k++){
						...
						if(teams.get(i).getZone().equals("W"))
				...
				/**筛选所有球队并输出*/
					...
					ArrayList<TeamVO> allTeams = dbl.getAllTeamInfo();
					...
					//teams = abl.sortTeam(team.n, allTeams, (String) field[0][0],(SortType) field[0][1]);
					...
					if(team.highData){
						...
						if(!team.avgValue){
							for(int k=0;k<teams.size();k++){
								...
								out.println(normalTeam.toString());
							}
						}
						else{
							for(int k=0;k<teams.size();k++){
...
class Player {
	...
	public void setSort(String field){
		...
		String[] temp = field.split(",");
		sortFields = new Object[temp.length][2];
		for(int i=0;i<temp.length;i++){
			String f = temp[i].split(".")[0];
			...
			switch(type){
			case "desc":
				sortFields[i][1] = SortType.Descending;
				break;
			case "asc":
				sortFields[i][1] = SortType.Ascending;
				break;
			default:
				sortFields[i][1] = SortType.Descending;
				break;
