...
public class ConnectMySQL {
		...
		try {
			...
			for(int i=season.length-1;i>=0;i--){
				...
				String sql3 = "CREATE TABLE player"+season[i]+"(id int unsigned not null auto_increment primary key,"
					+ "playerID char(20) not null,teamName char(20) not null,gamesPlayed int not null,gamesStarting int not null,rebounds double not null,offensiveRebounds double not null,"
				...
				String sql4 = "CREATE TABLE team"+season[i]+"(id int unsigned not null auto_increment primary key,"
					+ "teamID char(20) not null,teamName char(20) not null,gamesPlayed int not null,fieldGoalsMade double not null,fieldGoalsAttempted double not null,threePointFieldGoalsMade double not null,threePointFieldGoalsAttempted double not null,freeThrowsMade double not null,"
			...
			/*PreparedStatement pstmt;
			String sql5="CREATE TABLE allplayer (id int unsigned not null auto_increment primary key,"
					+ "playerID char(20) not null,playerName char(50) not null,playerNumber char(10) not null,position char(50) not null,height char(10) not null,weight char(10) not null,"
					+ "birth char(20) not null,age char(10) not null,exp char(20) not null,school char(50) not null)";
			String sql6="CREATE TABLE allteam (id int unsigned not null auto_increment primary key,"
					+ "teamID char(20) not null,teamName char(20) not null,fullName char(20) not null,city char(50) not null,zone char(20) not null,subarea char(20) not null,homeCourt char(50) not null,createTime char(20) not null)";
	...
	public void drop(){
		...
		try {
			...
			/*PreparedStatement pstmt;
			String sql5="DROP TABLE allplayer";
			String sql6="DROP TABLE allteam";
			pstmt = (PreparedStatement) conn.prepareStatement(sql5);
			pstmt.executeUpdate();
			pstmt = (PreparedStatement) conn.prepareStatement(sql6);
			pstmt.executeUpdate();
